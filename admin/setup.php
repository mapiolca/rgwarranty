...
/**
 * \file    gestionnairerg/admin/setup.php
 * \ingroup gestionnairerg
 * \brief   GestionnaireRG setup page.
 */
...
require_once '../lib/gestionnairerg.lib.php';
...
// Translations
$langs->loadLangs(array("admin", "gestionnairerg@gestionnairerg"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('gestionnairergsetup', 'globalsetup'));
...
$item = $formSetup->newItem('GESTIONNAIRERG_MYPARAM1');
$item->fieldParams['isMandatory'] = 1;
$item->fieldAttr['placeholder'] = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('GESTIONNAIRERG_MYPARAM2');
$item->defaultFieldValue = 'default value';
$item->fieldAttr['placeholder'] = 'A placeholder here';
$item->helpText = 'Tooltip text';

// Setup conf for selection of a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('GESTIONNAIRERG_MYPARAM3');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf for a selection of a Thirdparty
$item = $formSetup->newItem('GESTIONNAIRERG_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('GESTIONNAIRERG_MYPARAM5')->setAsYesNo();	 // ->fieldParams['alertifoff'] = 1 or ->fieldParams['alertifon'] = 1;

// Setup conf for a selection of an Email template of type thirdparty
$formSetup->newItem('GESTIONNAIRERG_MYPARAM6')->setAsEmailTemplate('thirdparty');

// Setup conf for a selection of a secured key
//$formSetup->newItem('GESTIONNAIRERG_MYPARAM7')->setAsSecureKey();

// Setup conf for a selection of a Product
$formSetup->newItem('GESTIONNAIRERG_MYPARAM8')->setAsProduct();
...
$formSetup->newItem('GESTIONNAIRERG_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('GESTIONNAIRERG_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('GESTIONNAIRERG_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('GESTIONNAIRERG_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf GESTIONNAIRERG_MYPARAM10
$item = $formSetup->newItem('GESTIONNAIRERG_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('GESTIONNAIRERG_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('GESTIONNAIRERG_MYPARAM12');
$item->fieldOverride = "Value forced, can't be modified";
$item->cssClass = 'minwidth500';

//$item = $formSetup->newItem('GESTIONNAIRERG_MYPARAM13')->setAsDate();	// Not yet implemented
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/gestionnairerg/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=gestionnairerg-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'GESTIONNAIRERG_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'GESTIONNAIRERG_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "GestionnaireRGSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-gestionnairerg page-admin');
...
$head = gestionnairergAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "gestionnairerg@gestionnairerg");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("GestionnaireRGSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'GESTIONNAIRERG_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'GESTIONNAIRERG_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
