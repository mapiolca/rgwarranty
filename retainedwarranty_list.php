...
/**
 *   	\file       retainedwarranty_list.php
 *		\ingroup    gestionnairerg
 *		\brief      List page for retainedwarranty
 */
...
include_once __DIR__.'/class/retainedwarranty.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('gestionnairerg', 'retainedwarranty', 'write');
	$permissiontodelete = $user->hasRight('gestionnairerg', 'retainedwarranty', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("gestionnairerg")) {
	accessforbidden('Module gestionnairerg not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'RetainedWarranty';
	$objectlabel = 'RetainedWarranty';
	$uploaddir = $conf->gestionnairerg->dir_output;
...
$title = $langs->trans("RetainedWarrantys");
//$help_url = "EN:Module_RetainedWarranty|FR:Module_RetainedWarranty_FR|ES:Módulo_RetainedWarranty";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/gestionnairerg/retainedwarranty_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-gestionnairerg page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/gestionnairerg/retainedwarranty_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendRetainedWarrantyRef";
$modelmail = "retainedwarranty";
$objecttmp = new RetainedWarranty($db);
