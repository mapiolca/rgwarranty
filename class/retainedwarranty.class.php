...
/**
 * \file        class/retainedwarranty.class.php
 * \ingroup     gestionnairerg
 * \brief       This file is a CRUD class file for RetainedWarranty (Create/Read/Update/Delete)
 */
...
/**
 * Class for RetainedWarranty
 */
class RetainedWarranty extends CommonObject
{
	/**
	 * @var string 		ID of module.
	 */
	public $module = 'gestionnairerg';

	/**
	 * @var string 		ID to identify managed object.
	 */
	public $element = 'retainedwarranty';

	/**
	 * @var string		Prefix to check for any trigger code of any business class to prevent bad value for trigger code.
	 * @see CommonTrigger::call_trigger()
	 */
	public $TRIGGER_PREFIX = 'GESTIONNAIRERG_MYOBJECT';	// Will be used to build trgiger keys 'GESTIONNAIRERG_MYOBJECT_MODIFY', ...

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'gestionnairerg_retainedwarranty';

	/**
	 * @var string 		If permission must be checked with hasRight('gestionnairerg', 'read') and not hasright('gestionnairerg', 'retainedwarranty', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'gestionnairerg';

	/**
	 * @var string 		String with name of icon for retainedwarranty. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'retainedwarranty@gestionnairerg' if picto is file 'img/object_retainedwarranty.png'.
	 */
	...
	//  */
	// public $table_element_line = 'gestionnairerg_retainedwarrantyline';
	...
	//  */
	// public $fk_element = 'fk_retainedwarranty';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'RetainedWarrantyline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'RetainedWarranty', 'fk_element'=>'fk_retainedwarranty'));
	...
	//  */
	// protected $childtablesoncascade = array('gestionnairerg_retainedwarrantydet');

	// /**
	//  * @var RetainedWarrantyLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('gestionnairerg', 'retainedwarranty', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('gestionnairerg', 'retainedwarranty', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('gestionnairerg', 'retainedwarranty_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'retainedwarranty/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'retainedwarranty/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'retainedwarranty/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'retainedwarranty/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->gestionnairerg->dir_output.'/retainedwarranty/'.$oldref;
				$dirdest = $conf->gestionnairerg->dir_output.'/retainedwarranty/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->gestionnairerg->dir_output.'/retainedwarranty/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('gestionnairerg','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('gestionnairerg','gestionnairerg_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'GESTIONNAIRERG_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'GESTIONNAIRERG_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'GESTIONNAIRERG_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowRetainedWarranty")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("RetainedWarranty").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$baseurl = dol_buildpath('/gestionnairerg/retainedwarranty_card.php', 1);
		$query = ['id' => $this->id];
		if ($option !== 'nolink') {
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowRetainedWarranty");
	...
	public function getLinesArray()
	{
		...
		$objectline = new RetainedWarrantyLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_retainedwarranty:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('GESTIONNAIRERG_MYOBJECT_ADDON')) {
			$conf->global->GESTIONNAIRERG_MYOBJECT_ADDON = 'mod_retainedwarranty_standard';
		}

		if (getDolGlobalString('GESTIONNAIRERG_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('GESTIONNAIRERG_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('GESTIONNAIRERG_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/gestionnairerg/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefRetainedWarranty $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			if (!empty($this->model_pdf)) {
				...
				$modele = getDolGlobalString('MYOBJECT_ADDON_PDF', 'standard_retainedwarranty');
			}
		}

		$modelpath = "core/modules/gestionnairerg/doc/";
...
/**
 * Class RetainedWarrantyLine. You can also remove this and generate a CRUD class for lines objects.
 */
class RetainedWarrantyLine extends CommonObjectLine
{
	// To complete with content of an object RetainedWarrantyLine
	// We should have a field rowid, fk_retainedwarranty and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'retainedwarranty'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_retainedwarranty'
