...
/**
 *	\file       htdocs/core/modules/gestionnairerg/doc/doc_generic_retainedwarranty_odt.modules.php
 *	\ingroup    gestionnairerg
 *	\brief      File of class to build ODT documents for retainedwarrantys
 */

dol_include_once('/gestionnairerg/core/modules/gestionnairerg/modules_retainedwarranty.php');
 ...
 */
class doc_generic_retainedwarranty_odt extends ModelePDFRetainedWarranty
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function __construct($db)
	{
		...
		$this->scandir = 'GESTIONNAIRERG_MYOBJECT_ADDON_PDF_ODT_PATH'; // Name of constant that is used to save list of directories to scan
	...
	public function info($langs)
	{
		...
		$odtPath = trim(getDolGlobalString('GESTIONNAIRERG_MYOBJECT_ADDON_PDF_ODT_PATH'));
		...
		$texte .= '<input type="hidden" name="param1" value="GESTIONNAIRERG_MYOBJECT_ADDON_PDF_ODT_PATH">';
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=gestionnairerg_retainedwarranty/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
				$texte .= ' &nbsp; <a class="reposition" href="'.$_SERVER["PHP_SELF"].'?modulepart=doctemplates&keyforuploaddir=GESTIONNAIRERG_MYOBJECT_ADDON_PDF_ODT_PATH&action=deletefile&token='.newToken().'&file='.urlencode(basename($file['name'])).'">'.img_picto('', 'delete').'</a>';
				$texte .= '<br>';
			}
			$texte .= '</div>';
		}
		...
		if (!getDolGlobalString('MAIN_NO_MULTIDIR_FOR_ODT')) {
			...
			$texte .= '<br>';
			$texte .= '<input type="hidden" name="value1" value="GESTIONNAIRERG_MYOBJECT_ADDON_PDF_ODT_PATH">';
		}
		...
		$texte .= '<input type="hidden" value="GESTIONNAIRERG_MYOBJECT_ADDON_PDF_ODT_PATH" name="keyforuploaddir">';
	 ...
	 *
	 *  @param	RetainedWarranty	$object				Source object to generate document from
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->gestionnairerg->dir_output) {
			// If $object is id instead of object
			if (!is_object($object)) {
				$id = $object;
				$object = new RetainedWarranty($this->db);
			...
			$dir = $conf->gestionnairerg->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->gestionnairerg->dir_temp);
				if (!is_writable($conf->gestionnairerg->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->gestionnairerg->dir_temp);
				...
				// Line of free text
				$newfreetext = '';
				$paramfreetext = 'GESTIONNAIRERG_MYOBJECT_FREE_TEXT';
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->gestionnairerg->dir_temp,
